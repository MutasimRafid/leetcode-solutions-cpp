...
public:
    vector<vector<int>> fourSum(vector<int>& num, int target) {
        ...
        //TC:O(N log N + N^3 logN) ... SC: O(M * 4), where M is the number of quads
        ...
        for(int i = 0;i<n;i++){
            for(int j = i+1;j<n;j++){
                for(int k = j+1;k<n;k++){
                    ...
                    //x is the element we are searching
        ...
        */
        ...
        Over here we will fix two-pointers and then find the remaining     
        two elements using two pointer technique as the array will be sorted at first.*/
